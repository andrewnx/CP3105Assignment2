...
if ( $info == 'saved' ) {
	echo '<div class="updated" id="message"><p><strong>' . esc_attr__( 'Item Added', 'floating-button' ) . '</strong>.</p></div>';
} elseif ( $info == 'update' ) {
	echo '<div class="updated" id="message"><p><strong>' . esc_attr__( 'Item Updated', 'floating-button' ) . '</strong>.</p></div>';
} elseif ( $info == 'delete' ) {
	$delid = absint( $_GET['did'] );
	$wpdb->delete( $data, [ 'id' => $delid ], [ '%d' ] );
	echo '<div class="updated" id="message"><p><strong>' . esc_attr__( 'Item Deleted', 'floating-button' ) . '</strong>.</p></div>';
}

$current_tab = ( isset( $_REQUEST["tab"] ) ) ? sanitize_text_field( $_REQUEST["tab"] ) : 'list';

$tabs = apply_filters( $this->plugin['slug'] . '_tab_menu', array(
	'list'      => esc_attr__( 'List', 'floating-button' ),
	'settings'  => esc_attr__( 'Add new', 'floating-button' ),
	'extension' => esc_attr__( 'Pro Features', 'side-menu' ),
	'tools'     => esc_attr__( 'Import/Export', $this->plugin['text'] ),
	'support'   => esc_attr__( 'Support', 'floating-button' ),
	'rate'      => esc_attr__( 'Rate', 'floating-button' ),
) );

$rating = 'https://wordpress.org/support/plugin/floating-button/reviews/#new-post';
?>
    ...
    <div class="wrap">
        ...
        <a href="?page=<?php echo esc_attr( $this->plugin['slug'] ); ?>&tab=settings" class="page-title-action">
			<?php esc_attr_e( 'Add New', 'floating-button' ); ?></a>
